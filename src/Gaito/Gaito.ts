...
export const GaitoInfo: SourceInfo = {
    ...
    websiteBaseURL: `https://www.gaito.io/truyen-hentai/`,
...
export class Gaito extends Source {
    getMangaShareUrl(mangaId: string): string { return `https://www.gaito.io/truyen-hentai/comic/${mangaId}` };
    ...
    async getMangaDetails(mangaId: string): Promise<Manga> {
        const url = `https://api.gaito.io/manga/comics/${mangaId}`;
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        const request = createRequestObject({
            url: `https://api.gaito.io/manga/chapters?comicId=${mangaId}&mode=by-comic&orderBy=bySortOrderDown`,
            method,
        });
        const data = await this.requestManager.schedule(request, 1);
        const json = (typeof data.data) === 'string' ? JSON.parse(data.data) : data.data;
    ...
    async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
        const request = createRequestObject({
            url: `https://api.gaito.io/manga/pages?chapterId=${chapterId}&mode=by-chapter`,
            method
        });
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        let request = createRequestObject({
            url: 'https://api.gaito.io/manga/comics?limit=20&offset=0&sort=latest', // get JSON not HTML
            method: "GET",
        });
        ...
        request = createRequestObject({
            url: 'https://api.gaito.io/manga/comics?limit=20&offset=0&sort=top-rated', // get JSON not HTML
            method: "GET",
        });
    ...
    async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        switch (homepageSectionId) {
            case "new_updated":
                url = `https://api.gaito.io/manga/comics?limit=20&offset=${page}&sort=latest`;
                select = 1;
                break;
            case "view":
                url = `https://api.gaito.io/manga/comics?limit=20&offset=${page}&sort=top-rated`;
                select = 2;
                break;
            default:
    ...
    async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        const request = createRequestObject({
            url: encodeURI(`https://api.gaito.io/manga/comics?genreId=${tags[0]}&limit=20&offset=${page}&sort=latest`),
            method: "GET",
        });
    ...
    async getSearchTags(): Promise<TagSection[]> {
        const tags: Tag[] = [];
        const url = `https://api.gaito.io/ext/genres?plugin=manga`
        const request = createRequestObject({
            url: url,
            method: "GET",
        });
