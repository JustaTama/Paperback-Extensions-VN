...
const DOMAIN = 'https://hentaivlz.com/'
const method = 'GET'
...
export const HentaiVLInfo: SourceInfo = {
    ...
    websiteBaseURL: `https://hentaivlz.com/`,
...
export class HentaiVL extends Source {
    getMangaShareUrl(mangaId: string): string { return `https://hentaivlz.com${mangaId}` };
    ...
    async getMangaDetails(mangaId: string): Promise<Manga> {
        const url = `https://hentaivlz.com${mangaId}`;
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        const request = createRequestObject({
            url: `https://hentaivlz.com${mangaId}`,
            method,
        });
    ...
    async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
        const request = createRequestObject({
            url: `https://hentaivlz.com${chapterId}`,
            method
        });
    ...
    async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        const request = createRequestObject({
            url: encodeURI(`https://hentaivlz.com${tags[0] ? tags[0] : ''}`),
            method: "GET",
            param: encodeURI(`?page=${page}`)
        });
